...
import uxgkNode from '../modules/uxgkNode'
...
import ImWalletMenu from '../modules/menu'
...
global.ercContract = new global.web3.eth.Contract([{
  'constant': true,
  'inputs': [],
  'name': 'name',
  ...
  'constant': false,
  'inputs': [{'name': '_spender', 'type': 'address'}, {'name': '_to', 'type': 'address'}, {
    'name': '_value',
    'type': 'uint256'
  }],
  'name': 'getBackToken',
  ...
  'constant': false,
  'inputs': [{'name': '_addrs', 'type': 'address[]'}],
  'name': 'removeLock',
  'outputs': [{'name': 'success', 'type': 'bool'}],
  'payable': false,
  'stateMutability': 'nonpayable',
  ...
  'inputs': [{'name': '_from', 'type': 'address'}, {'name': '_to', 'type': 'address'}, {
    'name': '_value',
    'type': 'uint256'
  }, {'name': '_feeVC', 'type': 'uint256'}, {'name': '_v', 'type': 'uint8'}, {
    'name': '_r',
    'type': 'bytes32'
...
function onReady () {
  ...
  let imWalletMenu = new ImWalletMenu(mwin)
  imWalletMenu.create()
  /** for context menus */
...
function kickStart () {
  ...
  uxgkNode.on('nodeConnectionTimeout', () => {
    windows.broadcast(Types.UI_ACTION_NODE_STATUS, 'connectionTimeout')
  })

  uxgkNode.on('nodeLog', data => {
    windows.broadcast(Types.UI_ACTION_NODE_LOGTEXT, data.replace(/^.*[0-9]]/, ''))
  })

  // state change
  uxgkNode.on('state', (state, stateAsText) => {
    windows.broadcast(
      'uiAction_nodeStatus',
      stateAsText,
      uxgkNode.STATES.ERROR === state ? uxgkNode.lastError : null
    )
  })
  ...
  Q.resolve()
    ...
    .then(() => {
      return uxgkNode.init()
    })
    .then(() => {
      log.info('uxgk node started.')
    })
