...
class NodeSync extends EventEmitter {
  constructor () {
    ...
    uxgkNode.on('state', _.bind(this._onNodeStateChanged, this))
  }
  ...
  start () {
    ...
    this._syncPromise = Q.try(() => {
      if (!(uxgkNode.state === uxgkNode.STATES.CONNECTED)) {
        throw new Error('Cannot sync - uxgkNode node not yet connected')
      }
  ...
  _onNodeStateChanged (state) {
    switch (state) { // eslint-disable-line default-case
      ...
      case uxgkNode.STATES.STOPPING:
        log.info('uxgk node stopping, so stop sync')

        this.stop()
        break
      // auto-sync whenever node gets connected
      case uxgkNode.STATES.CONNECTED:
        log.info('uxgk node connected, re-start sync')
